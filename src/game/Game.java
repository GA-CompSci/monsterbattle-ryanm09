...
public class Game {
    ...
    private void pickCharacterBuild() {
        ...
        if (choice == 0) {
            ...
            playerHeal -= (int) (Math.random() * 20 + 1) + 5;
            playerSpeed -= (int) (Math.random() * 6) + 3;
            ...
            playerDamage -= (int) (Math.random() * 21) + 5; //somehow this character is perfectly balanced on each difficulty already, awesome!
    ...
    private void handlePlayerAction(int action) {
        switch (action) {
            ...
            case 2: // Heal
                if (playerHealth >= 100) {
                    gui.displayMessage("Already max health!"); //no healing overflow
                } else {
                    heal();
                }
                break;
    ...
    private void attackMonster() {
        ...
        if (target.health() <= 0) {
            if (!playerSpecialActive && !chosenPlayerSpecial.equals("Malevolent Shrine")) { //this one kills some monsters instantly, don't want to chain these together
                specialKillCounter++;
